...
public class Ellipse extends Figure
{
	    ...
	    throws IllegalStateException
	{
		...
		ellipse.setRadiusX(Math.abs(radiusX)); ellipse.setRadiusY(Math.abs(radiusY));
	}
	...
	public Ellipse(Figure figure) throws IllegalArgumentException
	{
		...
		shape = new javafx.scene.shape.Ellipse(figureEllipse.getCenterX(), figureEllipse.getCenterY(), figureEllipse.getRadiusX(),figureEllipse.getRadiusY());
		root.getChildren().add(shape);
	...
	{
		return Math.max(getEllipseShape().getRadiusX(),getEllipseShape().getRadiusY());
	}
	...
	{
		double distanceX = getCenter().distance(lastPoint);
	...
	protected boolean equals(Figure figure)
	{
		...
		if (Math.abs(getRadius() - ellipse.getRadius()) > 1e-6)
		{
			return false;
		}
